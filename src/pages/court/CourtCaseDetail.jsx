    ...
    const fetchCaseDetails = async () => {
      try {
        ...
        if (caseRes.ok) {
          ...
          let timelineData = [];
          try {
            const tRes = await fetch(`${API_URL}/api/case/${id}/timeline`, {
              headers: { Authorization: `Bearer ${token}` }
            });
          ...
          let auditData = [];
          try {
            const aRes = await fetch(`${API_URL}/api/case/${id}/audit`, {
              headers: { Authorization: `Bearer ${token}` }
            });
          ...
          setCaseData({
            ...
            title: c.firId ? `${c.firId.category || 'Incident'} - ${c.firId.incidentLocation || 'Unknown'}` : 'CaseFile',
          ...
          if (c.courtProceedings) {
            const mappedOrders = c.courtProceedings.map(o => ({
              hearingDate: new Date(o.hearingDate).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
              }),
              note: o.note,
              nextHearingDate: o.nextHearingDate
                ? new Date(o.nextHearingDate).toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric'
                  })
              ...
              signedAt: new Date(o.signedAt).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
              }),
          ...
          if (c.evidence) {
            docs = c.evidence.map(e => ({
              ...
              uploadedBy: e.uploadedBy
                ? (e.uploadedBy.name || e.uploadedBy)
                : 'System',
          ...
          try {
            const docRes = await fetch(
              `${API_URL}/api/court/case/${c.caseId || c._id}/documents`,
              {
                headers: { Authorization: `Bearer ${token}` }
              }
            );
            ...
            if (docRes.ok) {
              ...
              const mappedCourtDocs = courtDocs.map(d => ({
                ...
                uploadedBy: d.uploadedBy
                  ? (d.uploadedBy.name || d.uploadedBy)
                  : 'Court',
    ...
    <div className="min-h-screen bg-[#FAF8F5] p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between animate-fade-in-up mb-4">
          <Link
            to="/court"
            className="flex items-center text-[#0B3D91] hover:text-[#0B3D91]/80 font-medium transition-colors"
          >
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[#1A1A1A]/70 text-sm max-w-2xl">
            ...
            {caseData.aiAnalysis && (
              <div className="bg-gradient-to-br from-indigo-900 to-violet-900 rounded-2xl shadow-lg p-6 mb-6 text-white animate-fade-in-up">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div className="bg-white/10 rounded-lg p-3">
                    ...
                    <p className="font-semibold">
                      {caseData.aiAnalysis.classification || 'Unknown'}
                    </p>
                  </div>
                  ...
                  <div className="bg-white/10 rounded-lg p-3">
                    ...
                    <p className="font-semibold">
                      {caseData.aiAnalysis.severity || 'Unknown'}
                    </p>
                  </div>
                </div>

                <div className="bg-white/10 rounded-lg p-4 mb-4">
                  <p className="text-indigo-200 text-xs uppercase font-bold mb-1">
                    Summary
                  </p>
                ...
                <div className="bg-white/10 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-sm leading-relaxed">
                    {caseData.aiAnalysis.severityReason ||
                      caseData.aiAnalysis.reasoning ||
                      'No reasoning provided.'}
                  </p>
                </div>

                {(caseData.aiAnalysis.keyInformation?.length > 0 ||
                  caseData.aiAnalysis.keywords?.length > 0) && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="bg-white/10 rounded-lg p-4">
                      ...
                      <ul className="list-disc list-inside text-sm space-y-1">
                        {caseData.aiAnalysis.keyInformation?.map(
                          (info, idx) => (
                            <li key={idx}>{info}</li>
                          )
                        )}
                    ...
                    <div className="bg-white/10 rounded-lg p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {caseData.aiAnalysis.keywords?.map((kw, idx) => (
                          <span
                            key={idx}
                            className="bg-indigo-800/50 px-2 py-1 rounded text-xs border border-indigo-500/30"
                          >
                            {kw}
                          </span>
                        ))}
                ...
                {caseData.aiAnalysis.investigationLeads?.length > 0 && (
                  <div className="bg-white/10 rounded-lg p-4 mb-4 border-l-4 border-emerald-400">
                    <p className="text-emerald-300 text-xs uppercase font-bold mb-2">
                      Investigation Leads
                    </p>

                    <ul className="list-disc list-inside text-sm space-y-1">
                      {caseData.aiAnalysis.investigationLeads?.map(
                        (lead, idx) => (
                          <li key={idx}>{lead}</li>
                        )
                      )}
                ...
                {caseData.aiAnalysis.riskIndicators?.length > 0 && (
                  <div className="bg-white/10 rounded-lg p-4 mb-4 border-l-4 border-red-400">
                    <p className="text-red-300 text-xs uppercase font-bold mb-2">
                      Risk Indicators
                    </p>

                    <ul className="list-disc list-inside text-sm space-y-1">
                      {caseData.aiAnalysis.riskIndicators?.map(
                        (risk, idx) => (
                          <li key={idx}>{risk}</li>
                        )
                      )}
                ...
                {caseData.aiAnalysis.entities &&
                  Object.keys(caseData.aiAnalysis.entities).length > 0 && (
                    <div className="bg-white/10 rounded-lg p-4">
                      <p className="text-indigo-200 text-xs uppercase font-bold mb-2">
                        Entities Extracted
                      </p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                        {Object.entries(caseData.aiAnalysis.entities).map(
                          ([type, list]) => {
                            if (!list || list.length === 0) return null;

                            return (
                              <div key={type}>
                                <span className="text-indigo-300 font-semibold capitalize block mb-1">
                                  {type}:
                                </span>
              ...
              {orders.length === 0 && caseStatus !== 'Disposed' ? (
                <div className="text-center p-8 bg-[#FAF8F5] rounded-xl border border-dashed border-gray-300">
                  ...
                  <p className="text-[#1A1A1A]/60">
                    No hearings or orders recorded yet.
                  </p>
                </div>
              ) : (
                      ...
                      <div className="bg-[#FAF8F5] p-4 rounded-xl border border-purple-100">
                        <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                          <h4 className="font-bold text-[#1A1A1A]">
                            Hearing on {order.hearingDate}
                          </h4>

                          {order.nextHearingDate &&
                            order.nextHearingDate !== 'TBD' && (
                        ...
                        <p className="text-sm text-[#1A1A1A]/80 mb-3 leading-relaxed">
                          {order.note}
                        </p>
                    ...
                    <div className="relative pl-6">
                      ...
                      <div className="bg-green-50 p-4 rounded-xl border border-green-200">
                        <h4 className="font-bold text-green-800 mb-1">
                          Final Judgment Passed - Case Disposed
                        </h4>

                        <p className="text-sm text-green-700 mb-3">
                          All proceedings closed. SMS alert broadcast to parties.
                        </p>
              ...
              <div className="overflow-x-auto">
                    ...
                    {documents.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className="font-mono text-[10px] text-gray-500 truncate block max-w-[120px]"
                            title={item.fileHash}
                          >
                            {item.fileHash || 'N/A'}
                          </span>
                        ...
                        <td className="px-4 py-3 text-right flex justify-end items-center space-x-2">
                          ...
                          <button
                            className="p-1.5 text-purple-600 hover:bg-purple-100 rounded transition-colors inline-block"
                            title="Download"
                          >
                ...
                {documents.length === 0 && (
                  <p className="text-center text-gray-500 py-6">
                    No evidence or documents attached yet.
                  </p>
                )}
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] p-3 rounded border border-gray-100 bg-[#FAF8F5] shadow-sm">
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100/50">
                          ...
                          <time className="text-[10px] text-gray-400 font-mono flex items-center">
                            <Clock className="w-3h-3 mr-1" />
                            {log.timestamp}
                          </time>
                        </div>
                      </div>
